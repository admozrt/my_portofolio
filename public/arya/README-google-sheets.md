# Setup Backend Google Sheets (RSVP & Ucapan) — Arya & Sekar

Undangan ini menyimpan **RSVP** dan **Ucapan** ke Google Sheets lewat **Google Apps Script Web App**
(tanpa server sendiri). Buat endpoint **baru** (terpisah dari undangan lain), lalu tempel URL-nya
ke `SHEETS_ENDPOINT` di `src/pages/wedding/WeddingPageAryaSekar.tsx`.

## Langkah

1. Buat **Google Sheet** baru. Buat **2 tab (sheet)**:
   - Tab `RSVP` — header baris 1: `Timestamp | Nama | Kehadiran | Jumlah | Pesan`
     (kolom *Kehadiran* bisa berisi **Hadir / Tidak Hadir / Masih Ragu**)
...
6. Buka `src/pages/wedding/WeddingPageAryaSekar.tsx`, isi:
   ```ts
