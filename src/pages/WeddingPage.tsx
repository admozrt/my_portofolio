...
// Returns 0→1 progress of how far el has been scrolled through
function useScrollProgress(ref: React.RefObject<HTMLElement | null>): number {
  ...
  useEffect(() => {
    const handle = () => {
      ...
      // scrollable = total height minus one viewport (sticky occupies 100vh)
      const scrollable = elHeight - viewH;
      if (scrollable <= 0) return;
      // 0 when top of el aligns with top of viewport, 1 when fully scrolled
...
.cover-btn:hover { color: var(--cream-100); }
...
.location-card {
  ...
  border-radius: 16px;
  overflow: hidden;
  box-shadow: 0 12px 48px rgba(62,39,35,0.08);
  border: 1px solid var(--cream-300);
}
...
export const WeddingPage: React.FC = () => {
  ...
  // Hero zoom: scale from 1.15 → 1 as user scrolls first 600px
  const heroScale = Math.max(1, 1.15 - (scrollY / 600) * 0.15);
  const heroOpacity = Math.max(0, 1 - scrollY / 800);

  // Which love story step is active (0-based, -1 = none)
  ...
  const handleOpen = useCallback(() => {
    ...
    // Auto-play music when cover opens
    if (audioRef.current) {
    ...
    <>
      ...
      <div className="wedding-root">
            ...
            <div className={`profile-card ${profileVisible ? "visible" : ""}`}>
              <div className="profile-photo">
                <div className="profile-placeholder">
                  <svg className="profile-placeholder-icon" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="stack-layout">
              ...
              <div className="stack-photo-col">
                {PHOTOS.map((src, i) => {
                  ...
                  const isEntered = stackProgress >= threshold;
        ...
        <section className="countdown-section" ref={countdownRef}>
          <div
            className={`animate-fade-in ${countdownVisible ? "visible" : ""}`}
          >
        ...
        <section className="location-section" ref={locationRef}>
          <div
            className={`animate-fade-in ${locationVisible ? "visible" : ""}`}
          >
        ...
        <section className="closing-section" ref={closingRef}>
          <Petals />
          <div
            className={`animate-fade-in ${closingVisible ? "visible" : ""}`}
          >
